...
export default class RestaurantController implements Crud {
    async create(request: Request, response: Response){
        const {name, adress, specialty} = request.body
        
        const isAnyUndefined = Citi.areValuesUndefined(name, adress, specialty);
        if(isAnyUndefined) return response.status(400).send();

        const newRestaurant = { name, adress, specialty};
    ...
    async update(request: Request, response: Response){
        ...
        const isAnyUndefined = Citi.areValuesUndefined(name, adress, specialty, id);
        if (isAnyUndefined) return response.status(400).send();

        const restaurantUpdated = {name, adress, specialty};

        const {httpStatus, messageFromUpdate} = await Citi.updateValue(Restaurant, id, restaurantUpdated);
